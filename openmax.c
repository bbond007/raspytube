...
{
    char formatStr[] = "ERROR:OpenMaxJPEG\n~5%s";
    char temp[strlen(formatStr) + strlen(errorStr)];
    snprintf(temp, sizeof(temp), formatStr, errorStr);
...
int portSettingsChanged(OPENMAX_JPEG_DECODER* decoder, size_t * outputWidth, size_t * outputHeight)
{
    ...
    size_t uWidth  = (unsigned int) portdef.format.image.nFrameWidth;
    size_t uHeight = (unsigned int) portdef.format.image.nFrameHeight;
...
int OpenMaxJPEG_setupDecoder(OPENMAX_JPEG_DECODER** pDecoder)
{
    ...
    while(toread > 0)
    {
        ...
        while((done == 0) || (decoder->pOutputBufferHeader == NULL))
        {
            if(decoder->pOutputBufferHeader == NULL)
            {
                ...
                if(ret == 0)
                {
                    portSettingsChanged(decoder, &outputWidth, &outputHeight);
                }
            }
            else
            {
